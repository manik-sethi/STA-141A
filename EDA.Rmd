...
integrated_list <- lapply(seq_along(sessions), function(i) {
  ...
  # (Optional) Compute decision type (not used in modeling predictors)
...
##################################################
# 3. Normalize Predictor: Normalize avg_spikes to [0, 1]
##################################################
...
ggplot(integrated_data, aes(sample = avg_spikes_boxcox)) +
  stat_qq(color = "steelblue") +
  stat_qq_line(color = "red") +
  facet_wrap(~ feedback_type) +
